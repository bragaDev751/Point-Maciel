...
export const CheckoutForm = ({
  ...
  // CÁLCULO DINÂMICO
  const multiplicador = Math.floor(pontosDisponiveis / 150);
  const percentualDesconto = multiplicador * 10;

const baseCalculo = tipoEntrega === "delivery"
  ? total + taxaEntrega
  : total;

const valorDescontoReal = baseCalculo * (percentualDesconto / 100);
  const pontosASubtrair = multiplicador * 150;

  const totalFinal = usarDescontoFidelidade
    ? Math.max(total - valorDescontoReal, 0)
    : total;
  ...
  const formatarWhats = () => {
    ...
    if (usarDescontoFidelidade) {
      msg += `*DESCONTO APLICADO:* ${percentualDesconto}% (-R$ ${valorDescontoReal.toFixed(2)})%0A`;
    }
    ...
    try {
      ...
      if (usarDescontoFidelidade) {
        const { error } = await supabase
          .from("fidelidade")
          .update({
            pontos_acumulados: pontosDisponiveis - pontosASubtrair,
          })
          .eq("tenant_id", TENANT_ID_MACIEL)
          .eq("cliente_telefone", telLimpo);
      ...
      <div className="bg-[#1a011a] p-6 rounded-[2.5rem] border border-white/10 space-y-6 shadow-2xl">
            ...
            {percentualDesconto > 0 && (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${usarDescontoFidelidade ? "bg-green-500 text-white" : "bg-yellow-400 text-black"}`}
                  >
                    <Gift size={20} />
                  </div>
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-white/40 uppercase">
                      Liberado {percentualDesconto}% de desconto (-R${" "}
                      {valorDescontoReal.toFixed(2)}){" "}
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="space-y-4 pt-4 border-t border-white/5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    <option value="" className="bg-[#1a011a]">
        ...
        <div className="pt-4 border-t border-white/10">
          ...
          <div className="flex justify-between items-end mb-6 px-2">
            <span className="text-[10px] font-black uppercase text-white/30 tracking-tighter">
              Total do Pedido
            </span>
              ...
              {usarDescontoFidelidade && (
                <>
                  ...
                  <span className="text-[10px] text-green-400 font-bold">
                    -R$ {valorDescontoReal.toFixed(2)}
                  </span>
                </>
              )}
              <div className="text-4xl font-black text-yellow-400 italic leading-none">
                R$ {totalFinalComTaxa.toFixed(2)}
              </div>
            </div>
          </div>
          ...
          >
            {isEnviando ? (
              ...
              <>
                <Smartphone size={22} /> Enviar Pedido
              </>
            )}
          </button>
        </div>
