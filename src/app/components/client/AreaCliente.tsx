...
export function AreaCliente() {
  ...
  const [dadosFidelidade, setDadosFidelidade] = useState<FidelidadeData | null>(
    null,
  );
  ...
  /* ================= CÁLCULOS DE DESCONTO PERCENTUAL ================= */
  const pontosAtuais = dadosFidelidade?.pontos_acumulados || 0;
  const multiplicador = Math.floor(pontosAtuais / 150);
  const percentualDesconto = multiplicador * 10;
  const pontosParaProximoNivel =
    pontosAtuais % 150 === 0 ? 150 : 150 - (pontosAtuais % 150);
  const resgatarCupom = () => {
    if (percentualDesconto <= 0) return;

    const mensagem = `Olá! Sou o(a) ${dadosFidelidade?.cliente_nome} e gostaria de resgatar meu cupom de ${percentualDesconto}% de desconto (${pontosAtuais} pontos acumulados) no meu pedido de hoje! 🎁`;
        ...
        <div className="relative z-10">
          <h2 className="text-white font-black uppercase text-2xl italic tracking-tighter mb-2">
            ...
            <span className="text-[#ffcc00]">
              {dadosFidelidade?.cliente_nome?.split(" ")[0] || "Visitante"}
            </span>
          </h2>
          <p className="text-white/40 text-[10px] uppercase font-bold tracking-[0.2em] mb-6">
            Acesse seu Point Club exclusivo
          </p>
        ...
        {buscaRealizada && (
          ...
          >
              ...
              <div className="relative bg-gradient-to-br from-[#2d012d] to-[#1a011a] p-8 rounded-[2.5rem] border border-white/10 overflow-hidden">
                {percentualDesconto > 0 && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="absolute inset-0 bg-[#ffcc00]/5 radial-gradient"
                  />
                )}

                <div className="flex justify-between items-start mb-10 relative z-10">
                  <div className="space-y-1">
                    <p className="text-[#ffcc00] text-[10px] font-black uppercase tracking-[0.3em]">
                      Membro VIP
                    </p>
                    <h3 className="text-white font-black italic text-xl uppercase tracking-tight">
                      Point Club Card
                    </h3>
                  </div>
                  <Trophy
                ...
                <div className="flex items-end justify-between mb-4 relative z-10">
                  <div>
                    ...
                    <span className="text-[#ffcc00] font-black ml-2 uppercase text-xs">
                      Pontos
                    </span>
                  </div>
                ...
                <div className="relative h-4 bg-black/50 rounded-full border border-white/5 p-1 mb-6">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.min(((pontosAtuais % 150) / 150) * 100, 100)}%`,
                    }}
                ...
                <AnimatePresence>
                  {percentualDesconto > 0 ? (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-[#ffcc00] p-4 rounded-2xl flex items-center justify-between shadow-[0_0_30px_rgba(255,204,0,0.3)]"
                    >
                      <div className="flex items-center gap-3">
                        <div className="bg-black text-[#ffcc00] p-2 rounded-lg font-black text-xs italic">
                          {percentualDesconto}% OFF
                        </div>
                        <p className="text-black font-black uppercase text-[10px] leading-tight">
                          Seu desconto atual <br /> é de {percentualDesconto}%!
                        </p>
                      </div>
                      <button
                        onClick={resgatarCupom}
                        className="bg-black text-white px-4 py-2 rounded-xl font-black text-[10px] uppercase hover:scale-105 transition-all active:scale-95"
                      >
                        Resgatar agora
                      </button>
                    </motion.div>
                    ...
                    <p className="text-white/40 text-[10px] font-bold uppercase italic tracking-wider text-center">
                      ...
                      pontos para liberar 10% de desconto
                    </p>
                  )}
                </AnimatePresence>
                {percentualDesconto > 0 && (
                  <p className="text-[#ffcc00]/60 text-[8px] font-black uppercase text-center mt-4 tracking-widest">
                    +150 pontos e você sobe para {percentualDesconto + 10}% de
                    desconto!
                  </p>
                )}
              </div>
            ...
            {extratoPontos.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-white font-black uppercase text-xs tracking-[0.2em] px-2 flex items-center gap-2">
                  <ReceiptText size={16} className="text-[#ffcc00]" /> Extrato
                  Point Club
                </h3>
                <div className="bg-[#1a011a] border border-white/5 rounded-[2rem] overflow-hidden">
                  {extratoPontos.map((item) => (
                    ...
                    >
                      ...
                      <span
                        className={`font-black text-sm italic ${item.quantidade > 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {item.quantidade > 0
                          ? `+${item.quantidade}`
                          : item.quantidade}
            ...
            <div className="space-y-5">
              <h3 className="text-white font-black uppercase text-xs tracking-[0.2em] px-2 flex items-center gap-2">
                <History size={16} className="text-[#ffcc00]" /> Últimas
                Experiências
              </h3>
              ...
              <div className="grid gap-6">
                {pedidosAntigos.map((pedido) => (
                  <motion.div
                    ...
                    className="bg-[#1a011a] border border-white/5 p-6 rounded-[2.5rem] shadow-xl"
                  >
                      ...
                      {pedido.itens?.map((item, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-sm font-black text-white uppercase italic leading-tight tracking-tight">
                                {item.nome}
                              </h4>
                              <p className="text-[10px] font-bold text-[#ffcc00] italic">
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-white/5">
                      <div>
                        ...
                        <p className="text-xl font-black text-white italic tracking-tighter">
                          R$ {pedido.total_pedido.toFixed(2)}
                        </p>
                      </div>
                      <button
                        onClick={() => (window.location.href = "/")}
                        className="bg-white/5 hover:bg-[#ffcc00] hover:text-black text-white p-4 rounded-2xl transition-all shadow-lg active:scale-90"
                      >
                        <RotateCcw size={20} />
                      </button>
                    </div>
