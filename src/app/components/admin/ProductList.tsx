...
export const ProductList = () => {
  ...
  const toggleStatus = async (id: string | number, statusAtual: boolean) => {
    try {
      ...
      setProdutos(prev => prev.map(p => p.id === id ? { ...p, disponivel: !statusAtual } : p));
    } catch {
  ...
  const deletar = async (id: string | number, imageUrl?: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from('produtos').delete().eq('id', id).eq('tenant_id', TENANT_ID_MACIEL);
      if (error) throw error;
  ...
  const produtosFiltrados = useMemo(() => {
    return produtos.filter(p => {
      ...
      const matchesCategoria = filtroCategoria === "Todas" || p.categoria_nome === filtroCategoria;
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all flex-shrink-0 ${
              filtroCategoria === "Todas" ? 'bg-[#ffcc00] text-[#3b013b]' : 'bg-white/5 text-white/40'
            }`}
          >
            🏠 Todas
          </button>
            ...
            .map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all flex-shrink-0 flex items-center gap-2 ${
                  filtroCategoria === cat ? 'bg-[#ffcc00] text-[#3b013b]' : 'bg-white/5 text-white/40'
                }`}
              >
          ...
          produtosFiltrados.map((p) => (
            <div
              ...
              className={`flex justify-between items-center p-5 rounded-[2.5rem] border transition-all group ${
                p.disponivel ? 'bg-white/5 border-white/5 hover:border-white/10' : 'bg-red-500/5 border-red-500/10 grayscale opacity-60'
              }`}
            >
              <div className="flex items-center gap-4 min-w-0">
                <button
                  ...
                  className={`w-12 h-12 flex items-center justify-center rounded-2xl transition-all flex-shrink-0 ${
                    p.disponivel ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20' : 'bg-red-500/20 text-red-500 hover:bg-red-500/30'
                  }`}
                >
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <p className={`font-bold text-base leading-tight truncate ${p.disponivel ? 'text-white/90' : 'text-white/30'}`}>
                      {p.nome}
                    </p>
                    {p.categoria_nome === "Artesanais" && (
                      <span className="bg-[#ffcc00] text-[#3b013b] text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter animate-pulse shadow-lg shadow-[#ffcc00]/20">
                        Premium
                      </span>
                    )}
                  </div>
