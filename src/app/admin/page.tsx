...
function ControleLoja() {
  ...
  if (loading)
    return <div className="w-28 h-8 bg-white/5 animate-pulse rounded-xl" />;
  ...
  return (
    ...
    >
      <span
        className={`w-2 h-2 rounded-full ${aberta ? "bg-green-500" : "bg-red-500 animate-pulse"}`}
      />
...
export default function AdminPage() {
  ...
  const carregarCategorias = useCallback(async () => {
    if (authLoading || !supabase || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
      return;
    try {
    ...
    return (
      <div className="min-h-screen bg-[#0f010f] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#ffcc00] font-black uppercase tracking-tighter text-[10px]">
            Autenticando Gabriel...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <main className="min-h-screen bg-[#0f010f] text-white p-4 md:p-8 w-full overflow-x-hidden">
      {" "}
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <header className="mb-12 flex flex-col xl:flex-row justify-between items-center gap-8">
          ...
          <nav className="flex bg-white/5 p-1.5 rounded-[2.5rem] border border-white/10 overflow-x-auto max-w-full no-scrollbar">
            {" "}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 w-full min-w-0">
                ...
                <div className="lg:col-span-4">
                  <div className="bg-white/5 p-6 rounded-3xl sticky top-8 border border-white/10">
                    <h3 className="text-[#ffcc00] font-black text-xs uppercase mb-4 tracking-widest">
                      Categorias
                    </h3>
                    <div className="flex gap-2 mb-4">
                      <input
                        ...
                        value={novaCat.emoji}
                        onChange={(e) =>
                          setNovaCat({ ...novaCat, emoji: e.target.value })
                        }
                      ...
                      <input
                        ...
                        value={novaCat.nome}
                        onChange={(e) =>
                          setNovaCat({ ...novaCat, nome: e.target.value })
                        }
                        className="flex-1 min-w-0 p-3  bg-black/40 rounded-xl border border-white/10 focus:border-[#ffcc00] outline-none transition-all"
                      />
                    </div>
                ...
                <div className="lg:col-span-8 space-y-10 w-full min-w-0">
                  {" "}
                  <ProductForm />
                  <ProductList />
                </div>
