...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    if (carrinhoSalvo) {
      try { setCarrinho(JSON.parse(carrinhoSalvo)); } catch { console.error("Erro no carrinho"); }
    }
...
const confirmarAdicaoModal = (p: Produto, qtdTotal: number, extras?: ComplementoSelecao[]) => {
  ...
  const produtosFiltrados = catAtiva === "Todos" 
    ? produtos 
    : produtos.filter((p) => p.categoria_nome?.trim() === catAtiva.trim());
  ...
  return (
    <>
      ...
      {etapa === "menu" ? (
        ...
        <CheckoutForm
          carrinho={carrinho} total={total} nome={nome} setNome={setNome}
          endereco={endereco} setEndereco={setEndereco} tipoEntrega={tipoEntrega}
          setTipoEntrega={setTipoEntrega} onAdd={handleAdd} onRemove={handleRemove}
          onBack={() => setEtapa("menu")} onConfirm={handleFinalizadoComSucesso} lojaAberta={lojaAberta}
        />
      )}
            ...
            {lojaAberta ? (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="bg-[#3b013b] text-[#ffcc00] w-8 h-8 rounded-xl flex items-center justify-center text-xs">{carrinho.length}</div>
              ...
              <div className="w-full bg-white/5 border border-white/10 p-4 rounded-[2rem] text-center backdrop-blur-md">
                <span className="text-[10px] font-black uppercase text-white/20 tracking-widest">Loja Fechada</span>
              </div>
            )}
          </div>
        </div>
